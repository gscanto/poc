...
try:
    ...
    print("⚠️ WeasyPrint não disponível. Exportação PDF desabilitada.")
...
def markdown_to_docx(markdown_text, title="Documento"):
    """Converte Markdown para DOCX com formatação"""
    doc = DocxDocument()
    ...
    while i < len(lines):
        ...
        if line.startswith('# '):
            ...
            run = heading.add_run(line[2:].strip())
            run.font.size = Pt(18)
            run.bold = True
        ...
        # Subtítulo (## )
            ...
            run = heading.add_run(line[3:].strip())
            run.font.size = Pt(14)
            run.bold = True
        ...
        # Subtítulo menor (### )
            ...
            run = heading.add_run(line[4:].strip())
            run.font.size = Pt(12)
            run.bold = True
        ...
        # Texto em negrito (**texto**)
            ...
            for idx, part in enumerate(parts):
                run = p.add_run(part)
                if idx % 2 == 1:  # Partes ímpares são negrito
                    run.bold = True
...
with tab1:
    ...
    <div class="info-box" style="margin-bottom: 1rem;">
        <b>📥 Formatos de Exportação Disponíveis:</b> Markdown (.md), Word (.docx), PDF (.pdf)
    </div>
    """, unsafe_allow_html=True)
    ...
    sections = st.text_area(
        "Defina as seções do documento (uma por linha)",
        value="Introdução\nMetodologia\nResultados\nConclusão",
        height=150,
        help="Digite o nome de cada seção em uma linha separada"
    )
            ...
            if not context.strip():
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # Gerar DOCX
                        docx_buffer = markdown_to_docx(markdown_content, title)
                        
                        # Gerar PDF (se disponível)
                        pdf_buffer = None
                        if WEASYPRINT_AVAILABLE:
                            pdf_buffer = markdown_to_pdf(markdown_content)
                        ...
                        with col_pdf:
                            if pdf_buffer:
                                ...
                                st.button(
                                    ...
                                    disabled=True,
                                    use_container_width=True,
                                    help="Instale weasyprint para habilitar: pip install weasyprint",
                                    key="download_pdf_disabled_btn"
                                )
                        
                        # Botão adicional para JSON completo
                        with st.expander("🔧 Opções Avançadas"):
                            st.download_button(
                                label="📋 Baixar JSON Completo (com metadados)",
